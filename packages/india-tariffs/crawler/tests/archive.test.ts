...
function makeArchive(): { archive: DocumentArchive; dir: string } {
...
function record(overrides: Partial<FetchRecord>, body: Buffer): FetchRecord {
  return {
...
test("put() recognizes the same binary observed at a new URL as one document", () => {
  ...
  const second = archive.put(
    body,
    record({ finalUrl: "https://example.gov.in/mirrors/a-copy.pdf" }, body),
    "TARIFF_ORDER",
  );
